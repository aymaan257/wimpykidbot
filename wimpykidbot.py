...
async def quote_cmd(interaction):
    if not is_locked(interaction):
        return await interaction.response.send_message("❌ Bot locked somewhere else.", ephemeral=True)
    await interaction.response.send_message(random.choice(quotes))

@tree.command(name="trivia", description="Play Wimpy Kid trivia", guild=discord.Object(id=GUILD_ID))
async def trivia_cmd(interaction):
    if not is_locked(interaction):
        return await interaction.response.send_message("❌ Bot locked somewhere else.", ephemeral=True)

...
def has_staff_role(interaction: discord.Interaction):
    return any(role.name.lower() == STAFF_ROLE_NAME.lower() for role in interaction.user.roles)
...
class TriviaButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.label == self.view_ref.correct:
            msg = await interaction.followup.send("✅ Correct!")
        else:
            msg = await interaction.followup.send(f"❌ Wrong! Answer: **{self.view_ref.correct}**")
...
async def trivia_cmd(interaction: discord.Interaction):
    ...
    wrong = random.sample([v for v in trivia_questions.values() if v != answer], 2)
    options = wrong + [answer]
    random.shuffle(options)

    view = TriviaView(answer, options)

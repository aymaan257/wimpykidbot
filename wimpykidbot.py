...
@tree.command(name="lock", description="Lock bot to this channel (staffs only)", guild=discord.Object(id=GUILD_ID))
...
@tree.command(name="unlock", description="Unlock bot (staffs only)", guild=discord.Object(id=GUILD_ID))
...
@tree.command(name="trivia", description="Play Wimpy Kid trivia!", guild=discord.Object(id=GUILD_ID))
async def trivia(interaction: discord.Interaction):
    if locked_channel_id and interaction.channel_id != locked_channel_id:
        await interaction.response.send_message("❌ Bot locked to another channel.", ephemeral=True)
        return
    ...
    async def send_question():
        ...
        wrong = [a for a in trivia_questions.values() if a != correct]
        ...
        class TriviaView(discord.ui.View):
            ...
            async def end(self, text):
                for c in self.children:
                    c.disabled = True
                await message.edit(content=text, view=self)
            ...
            async def stop(self, i, b):
                await self.end("🛑 Trivia stopped.")

            async def check(self, i, choice):
                if choice == correct:
                    ...
                    await i.response.defer()
                    await self.end(f"❌ Wrong! Answer: **{correct}**")

            async def on_timeout(self):
                await self.end(f"⏰ Time's up! Answer: **{correct}**")

        view = TriviaView()
        message = await interaction.followup.send(f"🧠 **Trivia Time!**\n{question}", view=view)
